    ...
    if GetJournalQuestType(journalQuestIndex) == QUEST_TYPE_CRAFTING then
      if GetJournalQuestNumSteps(journalQuestIndex) == QUEST_MAIN_STEP_INDEX then
        for numConditions = 1, GetJournalQuestNumConditions(journalQuestIndex, QUEST_MAIN_STEP_INDEX) do
          local conditionText, current, max, _, isComplete = GetJournalQuestConditionInfo(journalQuestIndex,
            QUEST_MAIN_STEP_INDEX, numConditions)
          ...
          if qtyToMove + stackCountInBag <= maxQtyAuthorized then
            ...
            if qtyToMove == stackSize or (qtyToMove < stackSize and qtyToMove + stackCountInBag == maxQtyAuthorized) then
              ...
              if stackCountInBag + qtyToMove <= maxQtyAuthorized then
                if destSlot then
                  itemMoved = moveItemInSlot(moveData.bagId, moveData.slotId, bagIdTo, destSlot, qtyToMove,
                    moveData.itemLink)
                elseif db.profiles[actualProfile].detailledNotMoved then
                  PrintItemsNotMovedToBag(moveData.itemLink, moveData.itemIcon, bagIdTo, qtyToMove)
                end
              ...
              if stackCountInBag + qtyToMove <= maxQtyAuthorized then
                ...
                if stackCountInBag + qtyToMove < db.profiles[actualProfile].rules[moveData.ruleName].onlyStacks * maxStack then
                  ...
                  if destSlot then
                    ...
                    itemMoved = moveItemInSlot(moveData.bagId, moveData.slotId, bagIdTo, destSlot,
                      stackSize - qtyToMove, moveData.itemLink)
...
local function moveCurrencies()
  ...
  if db.profiles[actualProfile].summary and (NonContiguousCount(moves) > 1 or (not db.profiles[actualProfile].detailledDisplay and NonContiguousCount(moves) == 1)) then
    ...
    for currencyType, side in pairs(moves) do
      if not message then
        message = GetString(BMR_ACTION_HAS_MOVED) .. zo_strformat(BMR_ACTION_CURRENCY_GSUMMARY,
          ZO_CurrencyControl_FormatCurrencyAndAppendIcon(amounts[currencyType], true, currencyType),
          GetString("BMR_ACTION_CURRENCY_MOVED_TO", side))
      else
        message = table.concat({ message, zo_strformat(BMR_ACTION_CURRENCY_GSUMMARY,
          ZO_CurrencyControl_FormatCurrencyAndAppendIcon(amounts[currencyType], true, currencyType),
          GetString("BMR_ACTION_CURRENCY_MOVED_TO", side)) }, GetString(SI_LIST_COMMA_SEPARATOR))
      end
    end
...
local function doInteractionWithGBank()
  ...
  if not inProgress then
    ...
    if #pushQueue > 0 then
      ...
      EVENT_MANAGER:RegisterForEvent(ADDON_NAME, EVENT_GUILD_BANK_ITEM_ADDED,
        function(_, errorReason) moveItems(true, errorReason) end)
    ...
    if keywordCondition[BMR_RULEWRITER_VALUE_WITH_OPERATOR] then
      ...
      if IsValidRuleWriterOperator(rawOperator) and rawValues then
        local isValidValuesForCondition, builtValues = GetValuesForCondition(keywordCondition,
          BMR_RULEWRITER_VALUE_WITH_OPERATOR, rawOperator, rawValues)
...
local function LAMSubmenu(subMenu)
  ...
  if subMenu == "currencies" then
    ...
    table.insert(submenuControls,
      { type = "description", text = zo_strformat("<<1>> :", GetCurrencyName(CURT_MONEY, false, false)) })
    ...
    table.insert(submenuControls,
      { type = "description", text = zo_strformat("<<1>> :", GetCurrencyName(CURT_TELVAR_STONES, false, false)) })
    ...
    table.insert(submenuControls,
      { type = "description", text = zo_strformat("<<1>> :", GetCurrencyName(CURT_ALLIANCE_POINTS, false, false)) })
    ...
    table.insert(submenuControls,
      panelCurrencyKeepInBankInstead("currency" .. CURT_ALLIANCE_POINTS, CURT_ALLIANCE_POINTS))
    table.insert(submenuControls,
      { type = "texture", image = "EsoUI/Art/Miscellaneous/horizontalDivider.dds", imageWidth = 510, imageHeight = 4 })

    table.insert(submenuControls,
      { type = "description", text = zo_strformat("<<1>> :", GetCurrencyName(CURT_WRIT_VOUCHERS, false, false)) })
  ...
  elseif subMenu == "blacksmithing" then
    ...
    table.insert(submenuControls, panelOnlyIfNotFullStack("improvement" .. CRAFTING_TYPE_BLACKSMITHING,
      "improvementAll" .. CRAFTING_TYPE_BLACKSMITHING))
    table.insert(submenuControls,
      panelGuildBank("improvement" .. CRAFTING_TYPE_BLACKSMITHING, "improvementGBank" .. CRAFTING_TYPE_BLACKSMITHING))
    table.insert(submenuControls,
      panelMaxStacks("improvement" .. CRAFTING_TYPE_BLACKSMITHING, "improvementAll" .. CRAFTING_TYPE_BLACKSMITHING,
    ...
    table.insert(submenuControls,
      panelRule("armorTypeTrait" .. ITEM_TRAIT_TYPE_ARMOR_INTRICATE .. "Weight" .. ARMORTYPE_HEAVY))
    ...
    table.insert(submenuControls,
      panelRule("weaponType" .. CRAFTING_TYPE_BLACKSMITHING .. "Trait" .. ITEM_TRAIT_TYPE_WEAPON_INTRICATE))
    table.insert(submenuControls, panelRule("weaponTypeResearchable" .. CRAFTING_TYPE_BLACKSMITHING))

    table.insert(submenuControls,
      { type = "texture", image = "EsoUI/Art/Miscellaneous/horizontalDivider.dds", imageWidth = 510, imageHeight = 4 })
    table.insert(submenuControls, panelOnlyIfNotFullStack("Material[A]?[l]?[l]?" .. CRAFTING_TYPE_BLACKSMITHING,
      "MaterialAll" .. CRAFTING_TYPE_BLACKSMITHING))
    table.insert(submenuControls, panelGuildBank("Material[A]?[l]?[l]?" .. CRAFTING_TYPE_BLACKSMITHING,
      "MaterialGBank" .. CRAFTING_TYPE_BLACKSMITHING))
    ...
    table.insert(submenuControls, panelMaxStacks("Material[A]?[l]?[l]?" .. CRAFTING_TYPE_BLACKSMITHING,
      "MaterialAll" .. CRAFTING_TYPE_BLACKSMITHING, "MaterialStacks" .. CRAFTING_TYPE_BLACKSMITHING))
  ...
  elseif subMenu == "clothier" then
    ...
    table.insert(submenuControls,
      panelOnlyIfNotFullStack("improvement" .. CRAFTING_TYPE_CLOTHIER, "improvementAll" .. CRAFTING_TYPE_CLOTHIER))
    table.insert(submenuControls,
      panelGuildBank("improvement" .. CRAFTING_TYPE_CLOTHIER, "improvementGBank" .. CRAFTING_TYPE_CLOTHIER))
    table.insert(submenuControls,
      panelMaxStacks("improvement" .. CRAFTING_TYPE_CLOTHIER, "improvementAll" .. CRAFTING_TYPE_CLOTHIER,
    ...
    table.insert(submenuControls,
      panelRule("armorTypeTrait" .. ITEM_TRAIT_TYPE_ARMOR_INTRICATE .. "Weight" .. ARMORTYPE_LIGHT))
    ...
    table.insert(submenuControls,
      panelRule("armorTypeTrait" .. ITEM_TRAIT_TYPE_ARMOR_INTRICATE .. "Weight" .. ARMORTYPE_MEDIUM))
    table.insert(submenuControls, panelRule("armorTypeResearchableWeight" .. ARMORTYPE_MEDIUM))
    table.insert(submenuControls,
      { type = "texture", image = "EsoUI/Art/Miscellaneous/horizontalDivider.dds", imageWidth = 510, imageHeight = 4 })

    table.insert(submenuControls, panelOnlyIfNotFullStack("Material[A]?[l]?[l]?" .. CRAFTING_TYPE_CLOTHIER,
      "MaterialAll" .. CRAFTING_TYPE_CLOTHIER))
    table.insert(submenuControls,
      panelGuildBank("Material[A]?[l]?[l]?" .. CRAFTING_TYPE_CLOTHIER, "MaterialGBank" .. CRAFTING_TYPE_CLOTHIER))
    table.insert(submenuControls,
      panelMaxStacks("Material[A]?[l]?[l]?" .. CRAFTING_TYPE_CLOTHIER, "MaterialAll" .. CRAFTING_TYPE_CLOTHIER,
        "MaterialStacks" .. CRAFTING_TYPE_CLOTHIER))
  ...
  elseif subMenu == "woodworking" then
    ...
    table.insert(submenuControls, panelOnlyIfNotFullStack("improvement" .. CRAFTING_TYPE_WOODWORKING,
      "improvementAll" .. CRAFTING_TYPE_WOODWORKING))
    table.insert(submenuControls,
      panelGuildBank("improvement" .. CRAFTING_TYPE_WOODWORKING, "improvementGBank" .. CRAFTING_TYPE_WOODWORKING))
    table.insert(submenuControls,
      panelMaxStacks("improvement" .. CRAFTING_TYPE_WOODWORKING, "improvementAll" .. CRAFTING_TYPE_WOODWORKING,
    ...
    table.insert(submenuControls,
      panelRule("weaponType" .. CRAFTING_TYPE_WOODWORKING .. "Trait" .. ITEM_TRAIT_TYPE_ARMOR_INTRICATE))
    table.insert(submenuControls, panelRule("weaponTypeResearchable" .. CRAFTING_TYPE_WOODWORKING))

    table.insert(submenuControls,
      { type = "texture", image = "EsoUI/Art/Miscellaneous/horizontalDivider.dds", imageWidth = 510, imageHeight = 4 })

    table.insert(submenuControls, panelOnlyIfNotFullStack("Material[A]?[l]?[l]?" .. CRAFTING_TYPE_WOODWORKING,
      "MaterialAll" .. CRAFTING_TYPE_WOODWORKING))
    table.insert(submenuControls,
      panelGuildBank("Material[A]?[l]?[l]?" .. CRAFTING_TYPE_WOODWORKING, "MaterialGBank" .. CRAFTING_TYPE_WOODWORKING))
    table.insert(submenuControls,
      panelMaxStacks("Material[A]?[l]?[l]?" .. CRAFTING_TYPE_WOODWORKING, "MaterialAll" .. CRAFTING_TYPE_WOODWORKING,
        "MaterialStacks" .. CRAFTING_TYPE_WOODWORKING))
  ...
  elseif subMenu == "jewelrycrafting" then
    ...
    table.insert(submenuControls, panelOnlyIfNotFullStack("improvement" .. CRAFTING_TYPE_JEWELRYCRAFTING,
      "improvementAll" .. CRAFTING_TYPE_JEWELRYCRAFTING))
    table.insert(submenuControls, panelGuildBank("improvement" .. CRAFTING_TYPE_JEWELRYCRAFTING,
      "improvementGBank" .. CRAFTING_TYPE_JEWELRYCRAFTING))
    table.insert(submenuControls,
      panelMaxStacks("improvement" .. CRAFTING_TYPE_JEWELRYCRAFTING, "improvementAll" .. CRAFTING_TYPE_JEWELRYCRAFTING,
    ...
    table.insert(submenuControls, panelOnlyIfNotFullStack("improvement" .. (CRAFTING_TYPE_JEWELRYCRAFTING * 100),
      "improvementAll" .. (CRAFTING_TYPE_JEWELRYCRAFTING * 100)))
    table.insert(submenuControls, panelGuildBank("improvement" .. (CRAFTING_TYPE_JEWELRYCRAFTING * 100),
      "improvementGBank" .. (CRAFTING_TYPE_JEWELRYCRAFTING * 100)))
    table.insert(submenuControls, panelMaxStacks("improvement" .. (CRAFTING_TYPE_JEWELRYCRAFTING * 100),
      "improvementAll" .. (CRAFTING_TYPE_JEWELRYCRAFTING * 100),
      "improvementStacks" .. (CRAFTING_TYPE_JEWELRYCRAFTING * 100)))
    for improvementItemIndex = 1, GetNumSmithingImprovementItems() do
      table.insert(submenuControls,
        panelRule("improvement" .. (CRAFTING_TYPE_JEWELRYCRAFTING * 100) .. improvementItemIndex))
    end

    table.insert(submenuControls,
      { type = "texture", image = "EsoUI/Art/Miscellaneous/horizontalDivider.dds", imageWidth = 510, imageHeight = 4 })

    table.insert(submenuControls, panelOnlyIfNotFullStack("Material[A]?[l]?[l]?" .. CRAFTING_TYPE_JEWELRYCRAFTING,
      "MaterialAll" .. CRAFTING_TYPE_JEWELRYCRAFTING))
    table.insert(submenuControls, panelGuildBank("Material[A]?[l]?[l]?" .. CRAFTING_TYPE_JEWELRYCRAFTING,
      "MaterialGBank" .. CRAFTING_TYPE_JEWELRYCRAFTING))
    table.insert(submenuControls, panelMaxStacks("Material[A]?[l]?[l]?" .. CRAFTING_TYPE_JEWELRYCRAFTING,
      "MaterialAll" .. CRAFTING_TYPE_JEWELRYCRAFTING, "MaterialStacks" .. CRAFTING_TYPE_JEWELRYCRAFTING))
  ...
  elseif subMenu == "cooking" then
    ...
    table.insert(submenuControls,
      panelMaxStacks("cookingIngredients", "cookingIngredientsAll", "cookingIngredientsStacks"))
...
local function buildLAMPanel()
  ...
  local optionsTable = {
    ...
    {
      type     = "submenu",
      name     = zo_strformat("<<1>> : <<2>>", GetString(SI_JOURNAL_MENU_QUESTS),
        GetString("SI_QUESTTYPE", QUEST_TYPE_CRAFTING)),
